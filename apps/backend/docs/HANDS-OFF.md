...
**Status:** Phase 0/1 ter-commit (`5c7ddb2`), Phase 2 (Quest) ter-commit
(`0e408ab`). **Phase 3 (Scoring) selesai** (kode + unit test + smoke test
end-to-end lolos; **belum di-commit**). MVP backend tinggal Phase 4 (Hardening).
...
- `contracts/openapi.yaml`: semua response 2xx dibungkus `{"data": ...}`
  (**ADR-025**); `QuestResponse` dapat `points` (**ADR-026**). Schema scoring
  (`ScoreResponse`/`StreakResponse`/`LeaderboardEntry`) tak diubah. Valid
...
- Berikutnya: `apps/backend/docs/tasks/phase-04-hardening.md`, **T4.1 → T4.8
  berurutan** → MVP backend selesai. Sebelum itu: commit Phase 3.
...
4. `docs/DECISIONS.md` — 27 ADR (ADR-016 kini **terisi**: atomicity =
   kompensasi manual). Relevan backend terbaru: **ADR-023** (registry
   error mapper), **ADR-024** (root Makefile + `.env.docker`), **ADR-025**
   (amplop response `{"data":...}`), **ADR-026** (`QuestResponse.points`).
5. `apps/backend/docs/tasks/phase-04-hardening.md` — task Phase 4 (berikutnya).
...
## Perangkap yang menunggu di Phase 4 (Hardening)

- **T4.1:** `server/health.go` sudah punya `healthHandler` + `readyHandler(db)`
  (readyz sudah `PingContext`). T4.1 tinggal memoles bila perlu.
- **T4.2–T4.4:** graceful shutdown penuh (`signal.NotifyContext` di `main.go`,
  `Server.Shutdown` + `db.Close`), `http.Server` timeouts lengkap, CORS.
  `main.go` saat ini belum graceful.
- **T4.5:** verifikasi tabel error mapping lengkap lintas module (mapping
  didaftar di tiap `New()` — user 3, quest 4, scoring 0).
- **T4.6:** cek kontrak vs implementasi 1:1. Sudah diketahui: `contracts`
  masih `servers: http://localhost:8080` & operasi `/me/score` dst punya
  respons `404` yang **tak dipakai** implementasi (wallet/streak auto-default).
  Putuskan: buang 404 dari kontrak atau biarkan (dokumentasikan).
- **T4.7:** `Dockerfile` — cocokkan versi Go (`1.25`) & module path; `go.sum`
  sudah ada.
- Lokal: server `:8001`, DB `:5433` (lihat `apps/backend/.env`); infra via
  root `make up`.
